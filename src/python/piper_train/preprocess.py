    ...
    with open(metadata_path, "r", encoding="utf-8") as csv_file:
        ...
        for row in reader:
            ...
            if not wav_path.exists():
                ...
                _LOGGER.warning("2Trying to set wav_path to:%s",wav_path)

            if not wav_path.exists():

                 # Try wav/ or wavs/
                wav_path = wav_dir / filename
                _LOGGER.warning("3Trying to set wav_path to:%s",wav_path)

            if not wav_path.exists():
                # Try with .wav
                wav_path = wav_dir / f"{filename}.wav"
                _LOGGER.warning("4Trying to set wav_path to:%s",wav_path)
